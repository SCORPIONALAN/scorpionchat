...
De igual forma en el contenedor ya existe comunicacion entre emisor y receptor, todo es UI. Pero basicamente el proceso es comparamos si el emisor del mensaje soy yo entonces estare mostrado a la derecha, y en caso de que el emisor del mensaje es la otra persona entonces se mostrara a la izquierda.
Tuve que crear una funcionalidad dentro de la carpeta de lib para unica y exclusivamente regresar el tiempo en formato HH:MM
Un problema de dejar la aplicacion asi es que si o si se necesita refrescar la pagina para cargar los mensajes en tiempo real, lo cual no lo hace tiempo real. Asi que en mi proximo avance estare usando SOCKET.IO

## Decimo dia
Tratando de entender SocketIO, basicamente es un servidor sobrepuesto en nuestro servidor de node, que estara encargado de administrar las conecciones asignandoles un ID unico, respecto a una cordenada URL de nuestro server.
Leyendo e investigando un poco sobre express, me percate que no soporta nativamente lo que viene siendo WebSockets, por lo que el servidor en vez de montarlo en express, lo estaremos montando en un servidor HTTP (libreria con el mismo nombre).
Con una instancia del servidor mediante la libreria de SocketIO podemos ver conexiones y desconecciones.
Cabe aclarar que esto es accion de cliente y del servidor, ya tenemos la parte del servidor que responde, pero ahora necesitaremos de SocketIO-client para la parte del cliente, ya que necesitaremos establecer sus conecciones con el servidor.
En useAuthStore se implemento una prop nueva exclusiva para el socket y 2 metodos para conectar el socket y desconectarlo. Ya una vez implementado estos metodos, haremos referencia a estos dentro de las lineas de codigo correspondientes para inciar sesion, crear cuenta y salir de la sesion
Posteriormente tuvimos que modificar el baackend dentro de los mensajes para mandar los mensajes a socketIO, con el metodo to() el argumento esperado es un id o un identificador al socket que le queremos mandar la informacion, posteriormente un emit() (por si solo es actualizar de forma broadcast) establecemos llave y valor para mandar el mensaje.
De igual forma en los entornos globales de autenticacion y del chat tuvimos que implementar funcionalidades de los sockets para indicar si estaban en linea, si habia actualizacion de mensajes (subscripciones y desubscripciones), conectar los sockets y desconectarlos.
Tambien para hacer pruebas hice un script para meter usuarios falsos hacer pruebas y optimizaciones.

## ULTIMO DIA
En este dia prepare los scripts para mandar el proyecto a un entorno de produccion en un servidor gratuito. De igual forma tuve que hacer el armado de la aplicacion de react para usarla como template dentro del backend. Las rutas de axios las tuve que cambiar, esto en base si esta en produccion o en desarrollo